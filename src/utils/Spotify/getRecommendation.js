...
const getRecommendation = async (search) => {
  ...
  try {
    ...
    const recommendationResult = await axios.get(
      `https://api.spotify.com/v1/recommendations?&seed_artists=${result.artistId}&seed_genres=romance%C2indie%2Cpop%2Cclassical&seed_tracks=${result.trackId}&limit=8`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
