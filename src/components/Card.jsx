...
import { Box, Center, useColorModeValue, Heading, Text, Stack, Image, Spacer, VStack, Flex } from '@chakra-ui/react';
...
const Card = ({ data, type, onClick }) => {
  ...
  return (
    <Center py={12} onClick={onCardClick} style={{ cursor: `${type === 'playlists' ? 'pointer' : ''}` }}>
      <Box role={'group'} p={6} maxW={'300px'} w={'full'} minH={'450px'} maxH={'450px'} bg={useColorModeValue('white', 'gray.800')} boxShadow={'2xl'} rounded={'lg'} pos={'relative'} zIndex={1}>
        <Box
          rounded={'lg'}
          mt={-12}
          pos={'relative'}
        ...
        >
          <Image rounded={'lg'} height={230} width={282} objectFit={'cover'} src={songData.image} alt="#" />
        </Box>
        <Flex direction="column" pt={20} height="100%">
          <Stack>
